...
- Added safe GitHub update lifecycle support. New tasks check the official `main` branch at most once per 24 hours in default `notify` mode; `Codex Science 업데이트` stages the exact advertised commit, verifies ancestry/runtime/the full tracked plugin cache, locks concurrent updates, atomically swaps the managed checkout, and retains the previous checkout if rollback cannot complete. `off` mode disables checks; unattended apply is intentionally unsupported. Dirty/custom/diverged checkouts are refused, the current task's loaded plugin cache is restored byte-for-byte, and fresh or repeated installer runs use staged/transactional validation.
