...
def _v2_tool() -> dict[str, Any]:
    return {
        ...
        "description": (
            "Execute a bounded typed public-source query and return normalized records plus a "
            "canonical request, page hashes, completeness, source release, and replay receipt."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
...
def _contracts_tool() -> dict[str, Any]:
    return {
        ...
        "description": "List typed public-source operations, query semantics, and maturity states.",
        ...
        if method == "initialize":
            return self._result(
                request_id,
                {
                    ...
                    "instructions": (
                        "Read-only scientific catalog and public-source queries. Prefer "
                        "science_query_source_v2 for material evidence so exact requests, "
                        "completeness, source releases, and replay hashes are retained."
                    ),
                },
            )
        if method == "ping":
        ...
        try:
            if name == "science_list_source_contracts":
                ...
                result = execute_connector(
                    self.connectors[str(source)], request, include_snapshot=include_snapshot
                )
                payload = result.to_dict(include_snapshot=include_snapshot)
            else:
