...
15. Run `"<plugin-root>/scripts/python_runtime.sh" "<plugin-root>/scripts/validate_artifact.py" <manifest>` before reporting completion. When a record/source review receipt is being emitted, add `--review-output <path> --require-passed-review` so unresolved sidecar findings fail the completion gate.
16. Run `"<plugin-root>/scripts/python_runtime.sh" "<plugin-root>/scripts/render_artifact_index.py" <manifest>` to create `index.md`. Add `--html` only when the user wants an offline browser view; `index.html` uses no hosted or external assets. Treat both indexes as derived navigation views, not evidence, and do not add them to the manifest.
