...
런타임은 순수 Python 표준 라이브러리로 동작하며 설치할 패키지가 없습니다. [`uv`](https://docs.astral.sh/uv/)는 개발 검증에만 필요합니다.
...
`bootstrap.sh`가 Python 버전을 확인하고 고정된 upstream 스킬 서브모듈을 얕게(shallow) 받아옵니다. clone 시 `--recurse-submodules`가 필요 없습니다. 설치 후에는 플러그인과 MCP 서버가 로드되도록 새 Codex 작업을 시작합니다.
...
개발 검사에는 [`uv`](https://docs.astral.sh/uv/)가 필요합니다. 결정론적 검사를 실행합니다.
...
Codex Science의 자체 코드는 [MIT License](LICENSE)로 배포됩니다. 가져온 스킬과 제3자 의존성에는 각각의 라이선스가 적용됩니다.
