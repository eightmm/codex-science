...
비단순 checkpoint는 현재 활성화의 owner key와 결합됩니다. Checkpoint가
`active`인데 Codex가 진행 보고만 남기고 끝내려 하면 `Stop` hook이 종료를
거부하고 저장된 `next_action`을 같은 턴에 다시 넣습니다. 같은 단계에서 실질적인
진전이 있으면 `heartbeat`를 기록합니다. Heartbeat나 상태 전이 없이 세 번 연속
종료를 시도하면 안전하게 빠져나오며, 각 run에는 기본 100회의 절대 continuation
예산도 있습니다. Codex 실행 전 `CODEX_SCIENCE_MAX_IDLE_CONTINUATIONS=1..20`을
설정하면 무진전 한도를 바꿀 수 있습니다. `approval_required`,
`waiting_external`, `blocked`에서는 턴 종료가 허용됩니다. 특히
`waiting_external`은 원격 job을 바쁘게 반복 조회하지 않고 poll 간격과 종료
규칙을 기록합니다.
...
대화 문맥을 통한 best-effort 지속만 가능하며 resume/compaction과 guarded
auto-continue는 보장되지 않습니다.
