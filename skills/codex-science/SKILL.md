...
5. Draft a short plan. Ask before package installation, new network hosts, remote compute, credentials, write-capable services, destructive actions, or expensive work. When the task requires running code or modeling, use `$cx-compute-environment` to build an isolated `uv` environment and execute in it — and specialized run skills such as `$cx-boltz-structure-prediction` for structure prediction. Obtain the gate approval once, then build the environment and run to completion.
