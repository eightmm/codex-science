...
For literature synthesis, use `$cx-literature-review` rather than ad-hoc parallel searches. Define the protocol and eligibility criteria, search complementary primary-literature sources, preserve exact query strings and dates, deduplicate by persistent identifier and underlying study, distinguish peer-reviewed articles from preprints and registry records, save a study table and evidence graph, and use `<plugin-root>/scripts/diff_literature_review.py` for living updates.
