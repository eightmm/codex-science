...
The plugin Stop guard mechanically rejects a progress-only final response while a session-owned checkpoint is `active` and feeds its next action back into the same turn. It allows the turn to stop for `approval_required`, `waiting_external`, `blocked`, `abandoned`, or `complete`. Three rejected stops without evidence-backed progress trigger an idle safety escape; the checkpoint's absolute continuation budget still applies after heartbeats and state transitions.
...
Do not combine Codex Science with another active generic Stop-loop such as Ralph Loop. Matching Stop hooks run concurrently and cannot reliably coordinate. Keep native Goal as the outer loop and the Codex Science Stop guard as the only inner auto-continue hook. Read [references/goal-loop-contract.md](references/goal-loop-contract.md) before operating a native Goal run or explaining its lifecycle.
