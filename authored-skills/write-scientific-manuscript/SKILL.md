...
10. **Package and validate.** Write `submission-package.json` with hashes for every included file. Run `"<plugin-root>/scripts/python_runtime.sh" "<plugin-root>/scripts/validate_manuscript_package.py" <package> --require-clean`. Resolve findings or keep the package in `draft`.
