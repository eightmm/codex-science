...
1. **Inspect the record.** Validate the source artifact manifest and review receipt. Inventory claims, supporting and contradicting evidence, null results, limitations, figures, tables, queries, executions, and unavailable material. Never reconstruct absent values from memory.
...
4. **Create the claim-citation map.** Assign manuscript claim IDs before prose expansion. Link each material claim to source claim IDs, hashed artifacts, persistent citation IDs, inference level, and a manuscript locator. Use `citation-needed` rather than inventing a reference.
5. **Draft evidence-near sections first.** Draft Methods and Results from recorded inputs, executions, analysis outputs, units, denominators, and uncertainty. Preserve failed, negative, null, and inconclusive outcomes.
...
8. **Apply reporting and venue constraints.** Complete `reporting-checklist.json`; use `unresolved` for missing items. Do not infer ethics, funding, authorship, conflicts, or availability statements.
...
10. **Package and validate.** Write `submission-package.json` with hashes for every included file. Run `python scripts/validate_manuscript_package.py <package> --require-clean`. Resolve findings or keep the package in `draft`.
...
12. **Review independently.** Use `$science-review` in `record`, `source`, and `method` modes as applicable. Set `submission-ready` only after a passed, hash-matched review receipt and resolution of every material finding.
